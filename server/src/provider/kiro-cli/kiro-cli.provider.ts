...
import { proxyHandler } from './kiro-cli.proxy-handler';
...
export class KiroCLIProvider implements Provider {
  ...
  async handleMessage(options: SendMessageOptions): Promise<void> {
    ...
    const sendRequest = async (token: string) => {
      const body = {
        conversationState: {
          conversationId: options.conversationId || require('crypto').randomUUID(),
    ...
    try {
      ...
      if (isExpired && tokens.refreshToken) {
        logger.info(`[KiroCLI] Token expired, refreshing...`);
        ...
        if (accountId) {
          try {
            ...
            logger.error('[KiroCLI] Failed to update refreshed tokens in DB:', e);
          }
        }
      ...
      if (payloadLength > 0) {
        try {
          const json = JSON.parse(
            buffer.slice(payloadOffset, payloadOffset + payloadLength).toString(),
          );
  ...
  async getModels(credential: string): Promise<any[]> {
    ...
    try {
      const parsed = JSON.parse(credential);
      tokens = { accessToken: parsed.access_token || parsed.accessToken };
    } catch (e) {
      tokens = { accessToken: credential };
    }
  ...
  async getUsage(credential: string): Promise<{ usage: string; resetPeriod: string }> {
    ...
    try {
      ...
      if (creditUsage) {
        ...
        const limit = creditUsage.usageLimitWithPrecision || 1;
        const percentage = ((currentUsage / limit) * 100).toFixed(1);
        return { usage: percentage, resetPeriod: 'month' };
      }
    } catch (e: any) {
