...
/**
 * Extracts explicit Session ID from CLI metadata (Claude Code or Qwen Code)
 */
function extractCliSessionId(body: any): string | null {
  const metadata = body.metadata;
  if (!metadata) return null;

  // 1. Qwen Code CLI format
  if (metadata.sessionId) return metadata.sessionId;

  // 2. Claude Code CLI format (usually inside user_id like user_...__session_UUID)
  if (metadata.user_id && typeof metadata.user_id === 'string') {
    const parts = metadata.user_id.split('__session_');
    if (parts.length > 1) {
      return parts[1];
    }
    // Deep check if it's embedded elsewhere
...
/**
 * Generates a unique session fingerprint based on explicit CLI Session IDs,
 * or falls back to API Key AND the content of the first conversation turn.
 * This allows multiple "independent" terminals to maintain separate backend sessions.
 */
function generateSessionFingerprint(
  ...
  // 1. Priority: Explicit CLI Session ID from metadata
  ...
  // 2. Fallback: Content-based fingerprinting for "headless" or legacy requests
  let firstUserMsg = '';
