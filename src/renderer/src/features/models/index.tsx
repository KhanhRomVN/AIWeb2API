...
import { FlatModel, SortKey, StatsPeriod, Provider } from './types';
...
export const ModelsPage = () => {
  ...
  const renderTableFilter = (current: StatsPeriod, setter: (p: StatsPeriod) => void) => (
    <Dropdown size="sm">
  ...
  return (
    <div className="h-full flex flex-row bg-background overflow-hidden">
      ...
      <div className="w-80 border-r border-border bg-card/30 flex flex-col shrink-0 h-full">
        {/* Sidebar Header */}
        ...
        <div className="h-12 flex items-center justify-between px-4 border-b border-border bg-card/50 backdrop-blur-sm shrink-0 relative z-20">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <button
                className="inline-flex items-center justify-center rounded-md text-xs font-medium border border-border bg-background hover:bg-accent hover:text-accent-foreground h-7 w-7 disabled:opacity-50 transition-colors"
                onClick={() => setOffset((prev: number) => Math.max(0, prev - 1))}
                disabled={offset === 0}
                title="Newer"
              >
